...
STATUS HashMap::lslSch(int obj_id, int key, int* value, LinkedHashNode** preds, 
								LinkedHashNode** currs, VALIDATION_TYPE val_type,
													int tid)
{
...
STATUS toValidation(int key, LinkedHashNode** currs, VALIDATION_TYPE val_type, int tid)
{
	...
	//getaptCurr
	if(key == currs[1]->key) curr = currs[1];

	else if(key == currs[0]->key) curr = currs[0];
	
	// NOTE: Sanity check for Default Time stamp of the
	//       node. Check if TS of node is not default.
	if((NULL != curr) && (key == curr->key))
	{
		//BTO read Time Stamp Validation.
		if((RV == val_type) && ((tid < curr->max_ts.ins_ts) ) )
		{
			op_status = ABORT;
		}
		//BTO tryComit validation.
		else if((tid < curr->max_ts.ins_ts) || (tid < curr->max_ts.look_ts) )
		{
			op_status = ABORT;
		}
		else
...
{
	//also required in BTO.
	STATUS op_status = interferenceValidation(preds, currs);

	//once done do time stamp validation for BTO.
	if(RETRY != op_status)
	{
...
STATUS HashMap::tidUpdate(int tid, int obj_id, int key)
{
	int bucket_num = HashFunc(key);
	...
	LinkedHashNode *entry = htable[bucket_num];
	while (entry->blue_next != NULL && entry->key != key)
	{
		entry->lmutex.lock();
		entry->max_ts.look_ts = tid;
		entry->rv_list->push_back(tid);
