	...
	static void concMiner( int t_ID, int numAUs, Graph *cGraph)
	{
		...
		while(curInd < numAUs)
		{
			...
			if (flag == true)
			{
				...
				//!------------------------------------------
				if(conf_list.begin() == conf_list.end())//!when conflist is empty.
				{
...
//			buffer.clear();
			...
			for(itr = buffer.begin(); itr != buffer.end(); itr++)
			{
				...
				if(temp->in_count == 0)
				{
					int expected = 0; //! expected in_degree is 0 then vertex can be executed if not claimed by other thread
					if(atomic_compare_exchange_strong( &(temp->in_count), &expected, -1 ) == true)
					{
						eAUCount++;                     //! num of Valid AUs executed is eAUCount+1

						//! get the AU to execute, which is of string type; listAUs index statrt with 0
						...
						{
							Graph::Graph_Node* refVN = (Graph::Graph_Node*)e_temp->ref;
							refVN->in_count--;
			...
			while(verTemp != cGraph->verTail)
			{
				if(verTemp->in_count == 0)
				{
					...
					if(atomic_compare_exchange_strong( &(verTemp->in_count), &expected, -1 ) == true)
					{
						...
						istringstream ss( listAUs[(verTemp->AU_ID) -1 ]);//get the AU to execute, which is of string type; listAUs index statrt with 0
						string tmp;
		...
		vTTime[t_ID] = vTTime[t_ID] + Ttimer._timeStop( start );//!stop timer to get time taken by this thread
	}
...
int main( )
{
	...
	cout<<pl<<"BTO Lock Free Decentralised Algorithm \n"<<pl;
	int totalRun = 6;
