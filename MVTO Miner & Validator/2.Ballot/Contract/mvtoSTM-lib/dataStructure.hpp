...
class version 
{
	public:
		...
		//To maintain largest trans id that
		//luckup in RV phase on this version.
		int G_max_RV;
		//used to store commited trans ids that
		//read/lookup on this version of the key.
...
public:
	...
	void list_Ins(int L_key, int* L_val, G_node** G_preds, G_node** G_currs, LIST_TYPE lst_type, int L_tx_id, voidVal* tb);
	...
	OPN_STATUS commonLuNDel(int L_tx_id, int L_bucket_id, int L_key, int* L_val, G_node** G_preds, G_node** G_currs, voidVal* tb);

	//Method to find the closest tuple created by transaction
	//Tj with the largest timestamp smaller than L_tx_id.
	...
	void insertVersion(int L_tx_id, int L_val, G_node* key_Node, list<int>&conf_list, voidVal* tb);
	...
	/*Method to update the time_stamp of current trans in rvl list of all the previous version this trans has read/written*/
	OPN_STATUS tidUpdate(int key, int L_tx_id, G_node* G_curr, int L_BucketId);
};
