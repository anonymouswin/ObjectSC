...
bool SimpleAuction::AuctionEnded_m( )
{
	...
	*hBider        = 0;
	...
	ops = lib->tx_read(txlog, maxBiderID+2, hBider, tb);//! highestBidder SObj
	if(ABORT == ops) return false;//AU aborted.
...
bool Coin::mint_m(int t_ID, int receiver_iD, int amount, int* time_stamp) 
{
	...
	*time_stamp = txlog->L_tx_id; //return time_stamp to user.
	...
	ops = lib->tx_read(txlog, CIDStart+receiver_iD, val, tb);
	if(ABORT != ops)
	{
		*val += amount;
		lib->tx_write(txlog, CIDStart+receiver_iD, *val, tb);
...
int Coin::send_m(int t_ID, int sender_iD, int receiver_iD, int amount, int *time_stamp, list<int>&conf_list) 
{
	...
	*Rval = *Rval + amount;
	...
	lib->tx_write(txlog, CIDStart+sender_iD, *Sval, tb);

	lib->tx_write(txlog, CIDStart+receiver_iD, *Rval, tb);
	txs   = lib->tryCommit( txlog, conf_list, tb);
	if(ABORT == txs) return 0;//AU aborted.
...
bool Coin::get_bal_m(int account_iD, int *bal, int t_ID, int *time_stamp, list<int>&conf_list) 
{
	...
	*time_stamp = txlog->L_tx_id; //return time_stamp to user.
	...
	ops = lib->tx_read(txlog, CIDStart+account_iD, val, tb);
...
int Ballot::giveRightToVote_m(int senderID, int voter)
{
	...
	ops = lib->tx_read(txlog, BIDStart+voter, val, cObj);
...
//		cout<<"\nVoter "+to_string(senderID)+" already voted or your not chairperson!\n";
	...
	lib->tx_write(txlog, BIDStart+voter, *val, cObj);
...
int Ballot::delegate_m(int senderID, int to, int *ts, list<int>&cList)
{
	...
	*ts   = txlog->L_tx_id;
	...
	Sops = lib->tx_read(txlog, BIDStart+senderID, val, sObj);
	if(ABORT == Sops) return 0; // AU transaction aborted.

	Tops = lib->tx_read(txlog, BIDStart+to, val, toObj);
...
//		lib->try_abort(T);
	...
	if( (*(Voter*)toObj->val).voted ) 
	{
		...
		OPN_STATUS Pops  = lib->tx_read(txlog, votedPropsal, val, pObj);
		...
		lib->tx_write(txlog, votedPropsal, *val, pObj);
	}
	...
	lib->tx_write(txlog, (*(Voter*)sObj->val).ID, *val, sObj);
...
int Ballot::vote_m( int senderID, int proposal, int *ts, list<int>&cList)
{
	...
	*ts   = txlog->L_tx_id;
	...
	Vops = lib->tx_read(txlog, BIDStart+senderID, val, vObj);
...
//		lib->try_abort(T);
	...
	Pops = lib->tx_read(txlog, BIDStart+numVoters+proposal, val, pObj);
	if(ABORT == Pops) return 0; // AU transaction aborted.
...
//	cout<<" new vote count is "+to_string((*(Proposal*)pObj->value).voteCount);
	lib->tx_write(txlog, BIDStart+senderID, *val, vObj);
	lib->tx_write(txlog, BIDStart+numVoters+proposal, *val, pObj);
...
void Ballot::state_m(int ID, bool sFlag)
{
	...
	if(sFlag == false)//Proposal
	{
		...
		Pops = lib->tx_read(txlog, BIDStart+numVoters+ID, val, Obj);
		if(ABORT == Pops)
		{
...
//		cout<<"================================\n";
	...
	if(sFlag == true)
	{
		...
		Pops = lib->tx_read(txlog, BIDStart+ID, val, Obj);
		if(ABORT == Pops)
		{
