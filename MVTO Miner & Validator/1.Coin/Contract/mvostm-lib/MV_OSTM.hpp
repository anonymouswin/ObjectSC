...
class MV_OSTM
{
	public:
		...
		OPN_STATUS tx_lookup(L_txlog* txlog, int L_key, int* value, voidVal* tb);
		/*Method to perform the delete operation over the hash table.*/
		OPN_STATUS tx_delete(L_txlog* txlog, int L_key, int* value, voidVal* tb);
		/*Method to perform the insert operation over the hash table.*/
		OPN_STATUS tx_insert(L_txlog* txlog, int L_key, int L_val, voidVal* tb);
