...
int Ballot::giveRightToVote_m(int senderID, int voter)
{
	...
	ops = lib->tx_lookup(txlog, voter, val, cObj);
...
//		lib->try_abort( );
	...
	ops = lib->tx_insert(txlog, voter, *val, cObj);
...
int Ballot::delegate_m(int senderID, int to, int *ts, list<int>&cList)
{
	...
	*ts   = txlog->L_tx_id;;
	...
	Sops = lib->tx_lookup(txlog, senderID, val, sObj);
	if(ABORT == Sops) return 0; // AU transaction aborted.

	Tops = lib->tx_lookup(txlog, to, val, toObj);
...
//		lib->try_abort(T);
	...
	if( (*(Voter*)toObj->val).voted ) 
	{
		...
		OPN_STATUS Pops = lib->tx_lookup(txlog, numVoters+votedPropsal, val, pObj);
		...
		lib->tx_insert(txlog, numVoters+votedPropsal, *val, pObj);
	}
	...
	lib->tx_insert(txlog, senderID, *val, sObj);
...
int Ballot::vote_m( int senderID, int proposal, int *ts, list<int>&cList)
{
	...
	*ts   = txlog->L_tx_id;
	...
	Vops = lib->tx_lookup(txlog, senderID, val, vObj);
...
//		lib->try_abort(T);
	...
	Pops = lib->tx_lookup(txlog, numVoters+proposal, val, pObj);
	if(ABORT == Pops) return 0; // AU transaction aborted.
...
//	cout<<" new vote count is "+to_string((*(Proposal*)pObj->value).voteCount);
	lib->tx_insert(txlog, senderID, *val, vObj);
	lib->tx_insert(txlog, numVoters+proposal, *val, pObj);
...
void Ballot::state_m(int ID, bool sFlag)
{
	...
	if(sFlag == false)//Proposal
	{
		...
		Pops = lib->tx_lookup(txlog, numVoters+ID, val, Obj);
		if(ABORT == Pops)
		{
...
//		cout<<"================================\n";
	...
	if(sFlag == true)
	{
		...
		Pops = lib->tx_lookup(txlog, ID, val, Obj);
		if(ABORT == Pops)
		{
