...
OPN_STATUS HashMap::list_LookUp(int L_bucket_id, int L_key, G_node** G_preds, G_node** G_currs)
{
	...
	#if DEBUG_LOGS
	cout<< endl<<"list_LookUp:: nodes " <<G_preds[0]->G_key<<" "
		<< G_preds[1]->G_key<<" "<<G_currs[0]->G_key<<" "<<G_currs[1]->G_key<<endl;
	#endif // DEBUG_LOGS
	return op_status;
}
...
//			cout<<"\nMax RV Time stamp of LVL "+to_string(closest_tuple->G_max_RV);
			//!=====================================================
		}
	}
...
bool HashMap::check_version(int L_tx_id, G_node* G_curr)
{
	...
	if(closest_tuple != NULL)
	{
		...
		for(int i=0; i<closest_tuple->G_rvl->size();i++)
		{
			...
			//int Tk_ts = closest_tuple->G_max_RV;
	...
	//If in the rv list all the transactions are of lower 
	//ts then the current transaction then return true.
	return true;
}

/**************************************************
 * Method to add a version in the appropriate 
 * key_node version list in sorted order.
 *************************************************/
void HashMap::insertVersion(int L_tx_id, int L_val, G_node* key_Node, list<int>&conf_list, voidVal* tb)
{
	...
	for(i = 0; i < key_Node->G_vl->size(); i++)
	{
		...
		if(key_Node->G_vl->at(i)->G_ts > L_tx_id)
		{
			...
			{
				auto it = key_Node->G_vl->at(j)->rList->begin();
				for(; it != key_Node->G_vl->at(j)->rList->end(); it++)
	...
	{
		auto it = key_Node->G_vl->at(i-1)->rList->begin();
		for(; it != key_Node->G_vl->at(i-1)->rList->end(); it++)
			conf_list.push_back(*it);
	}
...
/***********************************************
 * Function to delete a node from blue link
 * and place it in red link after marking it.
 **********************************************/
...
/*********************************
 * Function to release all locks 
 * taken during listLookUp().
 ********************************/
...
/*****************************************************
 * This method identifies the conflicts among the
 * concurrent methods of different transactions.
 ***************************************************/
