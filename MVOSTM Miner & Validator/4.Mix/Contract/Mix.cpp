...
int Ballot::giveRightToVote_m(int senderID, int voter)
{
	...
	ops = lib->tx_lookup(txlog, BIDStart+voter, val, cObj);
...
//		cout<<"\nVoter "+to_string(senderID)+" already voted or your not chairperson!\n";
	...
	lib->tx_insert(txlog, BIDStart+voter, *val, cObj);
...
int Ballot::delegate_m(int senderID, int to, int *ts, list<int>&cList)
{
	...
	*ts   = txlog->L_tx_id;
	...
	Sops = lib->tx_lookup(txlog, BIDStart+senderID, val, sObj);
	if(ABORT == Sops) return 0; // AU transaction aborted.

	Tops = lib->tx_lookup(txlog, BIDStart+to, val, toObj);
...
int Ballot::vote_m( int senderID, int proposal, int *ts, list<int>&cList)
{
	...
	*ts   = txlog->L_tx_id;
	...
	Vops = lib->tx_lookup(txlog, BIDStart+senderID, val, vObj);
...
//		lib->try_abort(T);
	...
	Pops = lib->tx_lookup(txlog, BIDStart+numVoters+proposal, val, pObj);
	if(ABORT == Pops) return 0; // AU transaction aborted.
...
//	cout<<" new vote count is "+to_string((*(Proposal*)pObj->value).voteCount);
	lib->tx_insert(txlog, BIDStart+senderID, *val, vObj);
	lib->tx_insert(txlog, BIDStart+numVoters+proposal, *val, pObj);
